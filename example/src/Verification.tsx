...
const VerificationScreen = ({ referenceID, onClose }: { referenceID: { referenceID: string }, onClose: () => void }) => {
  const { referenceNumber } = referenceID.referenceID || {};
  ...
  useEffect(() => {
    console.log(referenceID.referenceID,"+++++--------------------------+++++++",referenceNumber)
    if (referenceID.referenceID.replace(/^'|'$/g, '')) {
      const fetchVerificationData = async () => {
        try {
          const response = await fetch(
            `https://innotrust.innovitegra.online/api/enroll/${referenceID.referenceID.replace(/^'|'$/g, '')}`,
            {
              method: 'GET',
              headers: {
        ...
        {verificationData ? (
          <>
            <View style={styles.card}>
              ...
              <Text style={styles.value}>{referenceID.referenceID.replace(/^'|'$/g, '')}</Text>
            </View>
